...
class TradingWidget(QtWidgets.QFrame):
    """简单交易组件"""
    signal = QtCore.Signal(type(Event()))
    signalGatewayListChanged = QtCore.Signal(type(Event()))
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        if len(gatewayNameList) > 1:
            gatewayNameList.append('ALL')
        self.gatewayList.extend(gatewayNameList)

        self.initUi()
        self.connectSignal()
        self.eventEngine.register(EVENT_GATEWAY + 'ListChanged', self.signalGatewayListChanged.emit)
    ...
    def connectSignal(self):
        ...
        self.signalGatewayListChanged.connect(self.updateGatewayList)
    ...
    def sendOrder(self):
        ...
        logging.debug(
            "SendOrderReq{symbol:%s,exchange:%s,vtSymbol:%s,price:%s,volume:%s,direction:%s,priceType:%s,offset:%s,currency:%s,productClass:%s}" % \
            (req.symbol, req.exchange, req.vtSymbol, req.price, req.volume, self.comboDirection.currentText(),
             self.comboPriceType.currentText(), self.comboOffset.currentText(), req.currency, req.productClass))

        if gatewayName != 'ALL' and gatewayName != '':
            self.mainEngine.sendOrder(req, gatewayName)
        else:
            for g in self.gatewayList:
                if g != 'ALL' and g != '':
                    self.mainEngine.sendOrder(req, g)
                else:
                    logging.info("使用占位接口名称，不发送委托")
