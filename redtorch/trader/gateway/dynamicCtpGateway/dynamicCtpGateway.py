...
from redtorch.api.ctp import MdApi, TdApi, defineDict
from redtorch.trader.vtGateway import *
from redtorch.trader.vtFunction import getJsonPath, getTempPath
from redtorch.trader.vtConstant import GATEWAYTYPE_FUTURES
...
########################################################################
class DynamicCtpGateway(VtGateway):
    """CTP接口"""

    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName, userID, password, brokerID, tdAddress, mdAddress, authCode=None, userProductInfo=None):
        """Constructor"""
        super(DynamicCtpGateway, self).__init__(eventEngine, gatewayName)
    ...
    # ----------------------------------------------------------------------
    def isMdConnected(self):
        return self.mdConnected

    # ----------------------------------------------------------------------
    def isTdConnected(self):
        return self.tdConnected

    def changeConnectInfo(self, userID, password, brokerID, tdAddress, mdAddress, authCode=None, userProductInfo=None):
        self.userID = userID
        self.password = password
        self.brokerID = brokerID
        self.tdAddress = tdAddress
    ...
    def connect(self):
        ...
        try:
            ...
            self.tdApi.connect(self.userID, self.password, self.brokerID, self.tdAddress, self.authCode, self.userProductInfo)
            # 初始化并启动查询
            self.initQuery()
